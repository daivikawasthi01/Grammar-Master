...
const PlansContainer:React.FC<PlansContainerProps> = ({
    ...
    return (
        <div className={styles.plan}>
            ...
            <p>{desc}</p>
            <button className={planSelected === plan ? styles.plan__btn : styles.plan__btn__select__plan}>
                {btnName}
            </button>
            <div>
                {planFunctions.map((func, idx) => (
                    <p key={func + idx} className={styles.plan__functions}>
                        <svg className={styles.plan__icons} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
